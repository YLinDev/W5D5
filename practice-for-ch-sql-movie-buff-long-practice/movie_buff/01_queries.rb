...
def harrison_ford
  ...
  # as a lead actor.
  Actor.joins(:movies).select("movies.id", "movies.title").where("name = 'Harrison Ford' AND ord > 1")
end
...
def biggest_cast
  ...
  # actors).
  Actor.joins(:movies).select("movies.id", "movies.title").group("movies.id").order("COUNT(castings.actor_id) DESC").limit(3)
end
